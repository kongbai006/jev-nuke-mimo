...
基于 **nuke 脚本引擎**（ES Module / JS）的聊天辅助工具。对方私聊发消息后，**本地秒判**意图 / 危险等级 / 情绪（开心·难过·生气，含情绪强度与互动信号）；可选调用小米 MiMo 大模型生成候选回复，**手动确认才发送，绝不自动发消息**。一份脚本在 nuke 下同时适配微信和 QQ 私聊。
...
- 收到私聊文本 → 立刻弹窗显示：意图 / 危险等级(1-9) / 情绪百分比 / 情绪强度 / 互动信号；
