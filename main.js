...
// ============ ★ 在这里改设置 ★ ============
const CONFIG = {
    apiKey: "你的MiMo密钥填这里",          // sk- 开头，填完这行就能用
...
function quickAnalyze(convId, curText) {
    ...
    return { intent, danger, emotion: e.emotion, pct: e.pct };
}
...
async function callMimo(convId) {
    ...
    const sys = "你是高情商聊天助手。根据对话历史分析对方状态，严格只返回JSON：\n" +
        '{"intent":"意图","danger":1到9整数,"emotion":"开心/难过/生气","pct":0到100,' +
        '"action":"建议一句话","replies":["候选1","候选2","候选3"]}\n' +
        "replies 要口语自然、像真实聊天。" + rel;
    const messages = [{ role: "system", content: sys }, ...buildHistoryMessages(convId)];
